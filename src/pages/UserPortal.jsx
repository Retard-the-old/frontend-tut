  ...
  return (
    <div style={{ display:"flex", flexDirection:mob?"column":"row", minHeight:"100vh", background:"rgba(255,255,255,0.03)" }}>
      {!mob && <div style={{ width:230, background:"#080c0b", padding:"24px 0", display:"flex", flexDirection:"column", flexShrink:0 }}>
        ...
        <nav style={{ flex:1, padding:"12px 10px" }}>
          {navItems.map(function(n){ return (
            <button key={n.id} onClick={function(){gotoTab(n.id);setActiveLesson(null)}} style={{
              ...
              background: tab===n.id ? "rgba(0,228,193,0.15)" : "transparent", color: tab===n.id ? "#bdf9fc" : "#6ec9c4",
              fontSize:13, fontWeight: tab===n.id ? 600 : 500, cursor:"pointer", marginBottom:2
            }}><Ico name={n.icon} size={16} color={tab===n.id ? "rgb(0,228,193)" : "#3a7a74"} />{" "+n.label}</button>
          )})}
        </nav>
        <div style={{ padding:"12px 16px", borderTop:"1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
            ...
            <div><div style={{ fontSize:12, fontWeight:600, color:"#fff" }}>{u.name}</div><div style={{ fontSize:10, color:"#3a7a74" }}>{u.email}</div></div>
          </div>
          <button onClick={function(){ logout(); go("landing"); }} style={{ width:"100%", padding:"8px", borderRadius:6, border:"1px solid rgba(255,255,255,0.06)", background:"transparent", color:"#3a7a74", fontSize:12, cursor:"pointer" }}>Log Out</button>
        </div>
      </div>}
      {mob && <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 16px", background:"#080c0b", borderBottom:"1px solid rgba(255,255,255,0.06)", position:"sticky", top:0, zIndex:50 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <button onClick={function(){gotoTab("support")}} style={{ display:"flex", alignItems:"center", gap:5, padding:"6px 12px", borderRadius:6, border:"1px solid "+(tab==="support"?"rgba(0,228,193,0.3)":"rgba(255,255,255,0.06)"), background:tab==="support"?"rgba(0,228,193,0.08)":"transparent", cursor:"pointer" }}>
            <Ico name="chat" size={13} color={tab==="support"?"rgb(0,228,193)":"#6ec9c4"} />
            <span style={{ fontSize:11, fontWeight:600, color:tab==="support"?"rgb(0,228,193)":"#6ec9c4" }}>Support</span>
          </button>
      ...
      <div ref={contentRef} style={{ flex:1, padding:mob?16:28, overflow:"auto", maxHeight:"100vh", paddingBottom:mob?80:28, overflowX:"hidden", boxSizing:"border-box", minWidth:0, width:mob?"100%":"auto" }}>
        ...
        <div style={{ background:"linear-gradient(135deg, rgba(0,228,193,0.08), rgba(0,228,193,0.04))", border:"1px solid rgba(0,228,193,0.12)", borderRadius:14, padding:mob?"16px":"18px 24px", marginBottom:mob?16:24 }}>
          <div style={{ marginBottom:mob?8:12 }}>
            ...
            <div style={{ fontSize:mob?12:14, fontWeight:600, color:"#bdf9fc", fontFamily:"monospace", wordBreak:"break-all" }}>{"tutorii.com/ref/"+u.code}</div>
          </div>
          <button onClick={copyLink} style={{ background:"#0a0f0d", color:"#bdf9fc", border:"1px solid rgba(0,228,193,0.2)", padding:mob?"9px 0":"9px 18px", borderRadius:mob?20:8, fontSize:12, fontWeight:600, cursor:"pointer", width:mob?"100%":"auto", marginBottom:mob?12:12, display:mob?"block":"inline-block" }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            ...
            <button onClick={function(){window.open("sms:?body="+encodeURIComponent("Check out Tutorii - courses for expats + earn 40% on referrals! Join: https://tutorii.com/ref/"+u.code))}} style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:6, padding:"9px 12px", borderRadius:8, border:"none", background:"#3a7a74", color:"#fff", fontSize:11, fontWeight:700, cursor:"pointer" }}>
          ...
          <button onClick={function(){ window.open("/ref/"+u.code, "_blank"); }} style={{ width:"100%", marginTop:8, padding:"8px 12px", borderRadius:8, border:"1px dashed rgba(0,228,193,0.25)", background:"transparent", color:"#6ec9c4", fontSize:11, fontWeight:600, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
            <Ico name="link" size={11} color="#6ec9c4" /> Preview what your referral sees
          </button>
        </div>

        {tab === "overview" && <div>
          <h2 style={{ fontSize:22, fontWeight:700, margin:"0 0 20px", color:"#bdf9fc" }}>{"Welcome back, " + (u.name||"there").split(" ")[0]}</h2>
          ...
          <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"1fr 1fr", gap:14, alignItems:"stretch" }}>
            <div style={{ background:"#0a0f0d", borderRadius:14, padding:mob?14:22, border:"1px solid rgba(255,255,255,0.06)", boxSizing:"border-box" }}>
              <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 14px", color:"#bdf9fc" }}>Recent Referral Activity</h3>
              {(u.l1||[]).length === 0
                ? <div style={{ textAlign:"center", padding:"24px 0" }}><Ico name="link" size={28} color="#3a7a74" /><p style={{ fontSize:13, color:"#3a7a74", marginTop:10 }}>No referrals yet. Share your link to start earning.</p><span onClick={function(){gotoTab("referrals")}} style={{ fontSize:12, color:"rgb(0,228,193)", cursor:"pointer", fontWeight:600 }}>Get your referral link →</span></div>
                : (u.l1||[]).slice(0,4).map(function(r){ return (
                  <div key={r.name} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 0", borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                    <div><div style={{ fontSize:13, fontWeight:600, color:"#bdf9fc" }}>{r.name}</div><div style={{ fontSize:11, color:"#3a7a74" }}>{r.date}</div></div>
                    <div style={{ textAlign:"right" }}><div style={{ fontSize:13, fontWeight:500, color:"#bdf9fc" }}>{"AED "+(r.earned||0).toFixed(2)}</div><Badge s={r.status} /></div>
                  </div>
                )})
              }
            </div>
            <div style={{ background:"#0a0f0d", borderRadius:14, padding:mob?14:22, border:"1px solid rgba(255,255,255,0.06)", boxSizing:"border-box" }}>
              <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 14px", color:"#bdf9fc" }}>Continue Where You Left Off</h3>
              {courses.filter(function(c){return c.lessons.some(function(l){return !l.done})}).length === 0
                ? <div style={{ textAlign:"center", padding:"24px 0" }}><Ico name="book" size={28} color="#3a7a74" /><p style={{ fontSize:13, color:"#3a7a74", marginTop:10 }}>{courses.length === 0 ? "Courses loading..." : "All lessons complete! Great work."}</p></div>
                : courses.filter(function(c){return c.lessons.some(function(l){return !l.done})}).slice(0,3).map(function(c){
                  ...
                  return (
                    <div key={c.id} onClick={function(){gotoTab("courses");setOpenCourse(c.id);setActiveLesson(next)}} style={{ display:"flex", gap:12, alignItems:"center", padding:"10px 0", borderBottom:"1px solid rgba(255,255,255,0.04)", cursor:"pointer" }}>
                      ...
                      <div><div style={{ fontSize:13, fontWeight:600, color:"#bdf9fc" }}>{next ? next.title : ""}</div><div style={{ fontSize:11, color:"#3a7a74" }}>{c.module}</div></div>
                    </div>
                  );
                })
              }
          ...
          <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"1fr 1fr 1fr", gap:14, marginTop:14, alignItems:"stretch" }}>
              ...
              return (
                <div style={{ background:"linear-gradient(180deg, rgba(19,19,21,1) 0%, rgba(14,14,16,1) 100%)", borderRadius:16, padding:mob?14:24, border:"1px solid rgba(0,228,193,0.06)", boxShadow:"0 4px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.04)", boxSizing:"border-box" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                    <h3 style={{ fontSize:14, fontWeight:700, margin:0, color:"#bdf9fc" }}>Next Payout Date</h3>
                  ...
                  <div style={{ fontSize:28, fontWeight:500, color:"#bdf9fc", marginBottom:2 }}>{daysUntil === 1 ? "Tomorrow" : daysUntil + " days"}</div>
                  <div style={{ display:"inline-block", fontSize:10, color:"#3a7a74", marginBottom:14, padding:"3px 10px", borderRadius:6, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.05)", letterSpacing:0.3 }}>Tuesday via MamoPay</div>
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:"#3a7a74" }}>
            ...
            <div style={{ background:"linear-gradient(180deg, rgba(19,19,21,1) 0%, rgba(14,14,16,1) 100%)", borderRadius:16, padding:mob?14:24, border:"1px solid rgba(0,228,193,0.06)", boxShadow:"0 4px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.04)", boxSizing:"border-box" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                <h3 style={{ fontSize:14, fontWeight:700, margin:0, color:"#bdf9fc" }}>Monthly Profit</h3>
                ...
                return (
                  <div style={{ display:"flex", alignItems:"center", gap:16 }}>
                    <svg width="96" height="96" viewBox="0 0 96 96" style={{ flexShrink:0 }}><defs><filter id="glowRing" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="3" result="glow"/><feMerge><feMergeNode in="glow"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>
                      ...
                      <text x="48" y="44" textAnchor="middle" fill="#bdf9fc" fontSize="16" fontFamily="sans-serif" fontWeight="500">{"AED "+monthlyNet.toFixed(0)}</text>
                      <text x="48" y="58" textAnchor="middle" fill="#3a7a74" fontSize="8" fontFamily="sans-serif">PROFIT</text>
                    </svg>
                    <div>
                      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:6 }}>
                        <div style={{ width:8, height:8, borderRadius:2, background:"rgb(0,228,193)" }} />
                        <span style={{ fontSize:10, color:"#6ec9c4", padding:"2px 8px", borderRadius:5, background:"rgba(0,228,193,0.06)", border:"1px solid rgba(0,228,193,0.1)" }}>{"Earned AED "+monthlyGross.toFixed(2)}</span>
                      </div>
                      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:8 }}>
                        <div style={{ width:8, height:8, borderRadius:2, background:"rgba(248,113,113,0.4)" }} />
                        <span style={{ fontSize:10, color:"#6ec9c4", padding:"2px 8px", borderRadius:5, background:"rgba(248,113,113,0.05)", border:"1px solid rgba(248,113,113,0.1)" }}>{"Subscription -AED "+PRICE}</span>
                      </div>
                      <div style={{ display:"inline-block", fontSize:13, fontWeight:600, color:monthlyNet>=0?"rgb(0,228,193)":"#f87171", padding:"4px 12px", borderRadius:8, background:monthlyNet>=0?"rgba(0,228,193,0.08)":"rgba(248,113,113,0.08)", border:"1px solid "+(monthlyNet>=0?"rgba(0,228,193,0.15)":"rgba(248,113,113,0.15)"), marginTop:4 }}>{"Net Profit AED "+monthlyNet.toFixed(2)}</div>
                    </div>
                  </div>
                );
              })()}
            ...
            <div style={{ background:"linear-gradient(180deg, rgba(19,19,21,1) 0%, rgba(14,14,16,1) 100%)", borderRadius:16, padding:mob?14:24, border:"1px solid rgba(0,228,193,0.06)", boxShadow:"0 4px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.04)", boxSizing:"border-box" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                <h3 style={{ fontSize:14, fontWeight:700, margin:0, color:"#bdf9fc" }}>Referral Network Health</h3>
                ...
                return (
                  <div>
                    <div style={{ marginBottom:14 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:"#3a7a74", marginBottom:4 }}>
                      ...
                      <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:4 }}><div style={{ width:6, height:6, borderRadius:1, background:"rgb(0,228,193)" }} /><span style={{ fontSize:9, color:"#3a7a74", padding:"1px 6px", borderRadius:4, background:"rgba(0,228,193,0.06)", border:"1px solid rgba(0,228,193,0.08)" }}>{activeL1+" active"}</span></div>
                        <div style={{ display:"flex", alignItems:"center", gap:4 }}><div style={{ width:6, height:6, borderRadius:1, background:"rgba(248,113,113,0.4)" }} /><span style={{ fontSize:9, color:"#3a7a74", padding:"1px 6px", borderRadius:4, background:"rgba(248,113,113,0.05)", border:"1px solid rgba(248,113,113,0.08)" }}>{l1cancelled+" cancelled"}</span></div>
                      </div>
                    </div>
                    <div style={{ borderTop:"1px solid rgba(255,255,255,0.04)", paddingTop:12 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:"#3a7a74", marginBottom:4 }}>
                        <span>Network</span>
                        <span style={{ fontWeight:600, color:"#6ec9c4", fontSize:10, padding:"2px 8px", borderRadius:5, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.05)" }}>{((u.l1||[]).length + (u.l2||[]).length)+" total"}</span>
                      </div>
        ...
        {tab === "referrals" && <div>
          <h2 style={{ fontSize:22, fontWeight:700, margin:"0 0 20px", color:"#bdf9fc" }}>My Referrals</h2>
          ...
          <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"1fr 1fr", gap:14, marginBottom:16, alignItems:"stretch" }}>
            {/* Pie chart */}
            <div style={{ background:"linear-gradient(180deg, rgba(19,19,21,1) 0%, rgba(14,14,16,1) 100%)", borderRadius:16, padding:mob?14:24, border:"1px solid rgba(0,228,193,0.06)", boxShadow:"0 4px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.04)", boxSizing:"border-box" }}>
              <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 14px", color:"#bdf9fc" }}>Referral Network Breakdown</h3>
                ...
                <div>
                  {[{c:"rgb(0,228,193)",l:"Level 1 Active",v:activeL1},{c:"rgba(248,113,113,0.6)",l:"Level 1 Cancelled",v:(u.l1||[]).length-activeL1},{c:"rgba(167,139,250,0.7)",l:"Level 2",v:(u.l2||[]).length}].map(function(d){return (
                    <div key={d.l} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                      ...
                      <span style={{ fontSize:11, color:"#6ec9c4" }}>{d.l}</span>
                      <span style={{ fontSize:11, fontWeight:600, color:"#bdf9fc", marginLeft:"auto", padding:"1px 8px", borderRadius:5, background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.06)" }}>{d.v}</span>
                    </div>
                  )})}
                </div>
              </div>
            ...
            <div style={{ background:"linear-gradient(180deg, rgba(19,19,21,1) 0%, rgba(14,14,16,1) 100%)", borderRadius:16, padding:mob?14:24, border:"1px solid rgba(0,228,193,0.06)", boxShadow:"0 4px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.04)", boxSizing:"border-box" }}>
              <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 14px", color:"#bdf9fc" }}>Referral Growth Over Time</h3>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={(function(){
                  ...
                  <XAxis dataKey="month" tick={{fill:"#3a7a74",fontSize:10}} axisLine={false} tickLine={false} />
                  <YAxis tick={{fill:"#3a7a74",fontSize:10}} axisLine={false} tickLine={false} allowDecimals={false} />
                  <Tooltip contentStyle={{background:"rgba(10,10,12,0.95)",border:"1px solid rgba(0,228,193,0.15)",borderRadius:10,fontSize:11,color:"#bdf9fc",boxShadow:"0 8px 32px rgba(0,0,0,0.4)",padding:"8px 12px"}} itemStyle={{color:"#bdf9fc"}} cursor={{fill:"rgba(255,255,255,0.03)"}} />
                  ...
                  <Legend iconSize={10} wrapperStyle={{fontSize:11,color:"#6ec9c4",paddingTop:8}} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>

          <div style={{ background:"#0a0f0d", borderRadius:14, padding:mob?14:22, border:"1px solid rgba(255,255,255,0.06)", marginBottom:16, boxSizing:"border-box" }}>
            <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 14px", color:"#bdf9fc" }}>{"Level 1 - Direct (40% = AED "+(PRICE*L1_RATE).toFixed(2)+" each)"}</h3>
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              <thead><tr>{["Name","Joined","Status","Earned"].map(function(h){return <th key={h} style={{ padding:"8px 10px", fontSize:10, fontWeight:700, textTransform:"uppercase", color:"#3a7a74", textAlign:"left", borderBottom:"2px solid rgba(255,255,255,0.08)" }}>{h}</th>})}</tr></thead>
              <tbody>{(u.l1||[]).map(function(r){return <tr key={r.name} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)" }}><td style={{ padding:"10px", fontSize:13, fontWeight:600, color:"#bdf9fc" }}>{r.name}</td><td style={{ padding:"10px", fontSize:12, color:"#3a7a74" }}>{r.date}</td><td style={{ padding:"10px" }}><Badge s={r.status}/></td><td style={{ padding:"10px", fontSize:13, fontWeight:500, color:"#bdf9fc" }}>{"AED "+r.earned.toFixed(2)}</td></tr>})}</tbody>
            </table>
          </div>
          <div style={{ background:"#0a0f0d", borderRadius:14, padding:mob?14:22, border:"1px solid rgba(255,255,255,0.06)", boxSizing:"border-box" }}>
            <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 14px", color:"#bdf9fc" }}>{"Level 2 - Indirect (5% = AED "+(PRICE*L2_RATE).toFixed(2)+" each)"}</h3>
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              <thead><tr>{["Name","Via","Joined","Earned"].map(function(h){return <th key={h} style={{ padding:"8px 10px", fontSize:10, fontWeight:700, textTransform:"uppercase", color:"#3a7a74", textAlign:"left", borderBottom:"2px solid rgba(255,255,255,0.08)" }}>{h}</th>})}</tr></thead>
              <tbody>{(u.l2||[]).map(function(r){return <tr key={r.name} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)" }}><td style={{ padding:"10px", fontSize:13, fontWeight:600, color:"#bdf9fc" }}>{r.name}</td><td style={{ padding:"10px", fontSize:12, color:"#bdf9fc" }}>{r.from}</td><td style={{ padding:"10px", fontSize:12, color:"#3a7a74" }}>{r.date}</td><td style={{ padding:"10px", fontSize:13, fontWeight:500, color:"#a78bfa" }}>{"AED "+r.earned.toFixed(2)}</td></tr>})}</tbody>
            </table>
          </div>
        </div>}

        {tab === "earnings" && <div>
          <h2 style={{ fontSize:22, fontWeight:700, margin:"0 0 20px", color:"#bdf9fc" }}>Earnings</h2>
          <div style={{ display:"grid", gridTemplateColumns:mob?"1fr 1fr":"repeat(4, 1fr)", gap:mob?10:14, marginBottom:mob?16:20, alignItems:"stretch" }}>
            <StatCard icon="dollar" label="Total Earnings" value={"AED "+u.earn.total} />
            <StatCard icon="chart" label="This Month's Earnings" value={"AED "+u.earn.month} />
          ...
          <div style={{ background:"linear-gradient(180deg, rgba(19,19,21,1) 0%, rgba(14,14,16,1) 100%)", borderRadius:16, padding:mob?14:24, border:"1px solid rgba(0,228,193,0.06)", boxShadow:"0 4px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.04)", boxSizing:"border-box", marginBottom:16 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
              <h3 style={{ fontSize:14, fontWeight:700, margin:0, color:"#bdf9fc" }}>Weekly Earnings Breakdown</h3>
              <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
                {[["4w",mob?"4W":"Last 4 Weeks"],["3m",mob?"3M":"Last 3 Months"],["all","All"]].map(function(r){ return (
                  <button key={r[0]} onClick={function(){setChartRange(r[0])}} style={{ padding:mob?"4px 8px":"4px 10px", borderRadius:6, border:"1px solid "+(chartRange===r[0]?"rgba(0,228,193,0.3)":"rgba(255,255,255,0.06)"), background:chartRange===r[0]?"rgba(0,228,193,0.1)":"transparent", fontSize:10, fontWeight:600, color:chartRange===r[0]?"rgb(0,228,193)":"#3a7a74", cursor:"pointer" }}>{r[1]}</button>
                )})}
              </div>
            </div>
            <p style={{ fontSize:11, color:"#3a7a74", margin:"0 0 16px" }}>Weekly commission breakdown — Level 1 direct, Level 2 indirect, and net profit</p>
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={(function(){ var h=u.earningsHistory||[]; return chartRange==="4w"?h.slice(-4):chartRange==="3m"?h.slice(-12):h; })()} margin={{top:5,right:5,left:mob?-25:-10,bottom:5}}>
                ...
                <XAxis dataKey="week" tick={{fill:"#3a7a74",fontSize:10}} />
                <YAxis tick={{fill:"#3a7a74",fontSize:10}} tickFormatter={function(v){return "AED "+v}} />
                <Tooltip contentStyle={{background:"#0a0f0d",border:"1px solid rgba(0,228,193,0.2)",borderRadius:8,fontSize:12,color:"#bdf9fc"}} labelStyle={{color:"#bdf9fc",marginBottom:4}} itemStyle={{color:"#bdf9fc"}} formatter={function(v){return "AED "+v.toFixed(2)}} />
          ...
          <div style={{ background:"linear-gradient(180deg, rgba(19,19,21,1) 0%, rgba(14,14,16,1) 100%)", borderRadius:16, padding:mob?14:24, border:"1px solid rgba(0,228,193,0.06)", boxShadow:"0 4px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.04)", boxSizing:"border-box" }}>
            <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 16px", color:"#bdf9fc" }}>Monthly Earnings Breakdown</h3>
          ...
          <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"2fr 1fr", gap:14, marginTop:16, alignItems:"stretch" }}>
            <div style={{ background:"linear-gradient(180deg, rgba(19,19,21,1) 0%, rgba(14,14,16,1) 100%)", borderRadius:16, padding:mob?14:24, border:"1px solid rgba(0,228,193,0.06)", boxShadow:"0 4px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.04)", boxSizing:"border-box" }}>
              <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 4px", color:"#bdf9fc" }}>Total Earnings Over Time</h3>
              <p style={{ fontSize:11, color:"#3a7a74", margin:"0 0 16px" }}>Running total of all commissions earned since you joined</p>
              <ResponsiveContainer width="100%" height={240}>
                <AreaChart margin={{top:5,right:5,left:mob?-25:-10,bottom:5}} data={(function(){
                  ...
                  <CartesianGrid strokeDasharray="4 6" stroke="rgba(255,255,255,0.04)" strokeWidth={0.5} />
                  <XAxis dataKey="week" tick={{fill:"#3a7a74",fontSize:9}} interval={3} />
                  <YAxis tick={{fill:"#3a7a74",fontSize:10}} tickFormatter={function(v){return "AED "+v}} />
                  <Tooltip contentStyle={{background:"#0a0f0d",border:"1px solid rgba(0,228,193,0.2)",borderRadius:8,fontSize:12,color:"#bdf9fc"}} labelStyle={{color:"#bdf9fc",marginBottom:4}} itemStyle={{color:"#bdf9fc"}} formatter={function(v){return "AED "+v.toFixed(2)}} />
            ...
            <div style={{ background:"linear-gradient(180deg, rgba(19,19,21,1) 0%, rgba(14,14,16,1) 100%)", borderRadius:16, padding:mob?14:24, border:"1px solid rgba(0,228,193,0.06)", boxShadow:"0 4px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.04)", boxSizing:"border-box", display:"flex", flexDirection:"column" }}>
              <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 16px", color:"#bdf9fc" }}>Projected Annual Earnings</h3>
              {(function(){
                var monthlyGross = activeL1 * PRICE * L1_RATE + (u.l2||[]).length * PRICE * L2_RATE;
                var monthlyNet = monthlyGross - PRICE;
                ...
                return (
                  <div style={{ flex:1, display:"flex", flexDirection:"column", justifyContent:"space-between" }}>
                    <div>
                      <div style={{ display:"inline-block", fontSize:9, fontWeight:700, color:"#3a7a74", marginBottom:8, padding:"3px 10px", borderRadius:5, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.05)", letterSpacing:0.8 }}>IF CURRENT RATE HOLDS</div>
                      <div style={{ fontSize:28, fontWeight:500, color:annualNet>=0?"rgb(0,228,193)":"#f87171", marginBottom:4 }}>{"AED "+annualNet.toFixed(2)}</div>
                      <div style={{ display:"inline-block", fontSize:9, color:"#3a7a74", marginTop:4, padding:"2px 8px", borderRadius:4, background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.04)", letterSpacing:0.3 }}>{"net per year · " + activeL1 + " active L1 · " + (u.l2||[]).length + " L2"}</div>
                    </div>
                    <div style={{ borderTop:"1px solid rgba(255,255,255,0.04)", paddingTop:14, marginTop:14 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                        <span style={{ fontSize:10, color:"#3a7a74", padding:"1px 6px", borderRadius:4, background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.04)" }}>Gross annual</span>
                        <span style={{ fontSize:11, fontWeight:600, color:"#bdf9fc" }}>{"AED "+(monthlyGross*12).toFixed(2)}</span>
                      </div>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                        <span style={{ fontSize:10, color:"#3a7a74", padding:"1px 6px", borderRadius:4, background:"rgba(248,113,113,0.04)", border:"1px solid rgba(248,113,113,0.06)" }}>Subscription cost</span>
                      ...
                      <div style={{ display:"flex", justifyContent:"space-between", paddingTop:6, borderTop:"1px solid rgba(255,255,255,0.04)" }}>
                        <span style={{ fontSize:10, fontWeight:600, color:"#6ec9c4", padding:"1px 6px", borderRadius:4, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.05)" }}>Net annual</span>
        ...
        {tab === "payouts" && <div style={{ maxWidth:"100%", overflow:"hidden" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
            <h2 style={{ fontSize:22, fontWeight:700, margin:0, color:"#bdf9fc" }}>Payouts</h2>
            <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap", justifyContent:mob?"flex-start":"flex-end" }}>
              <span style={{ fontSize:12, color:u.earn.pending >= 50 ? "#6ec9c4" : "#3a7a74" }}>
          ...
          {withdrawOpen && (
            <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }} onClick={function(e){if(e.target===e.currentTarget)setWithdrawOpen(false)}}>
              <div style={{ background:"#0a0f0d", border:"1px solid rgba(255,255,255,0.08)", borderRadius:16, padding:28, width:"100%", maxWidth:400 }}>
                <div style={{ fontSize:18, fontWeight:700, color:"#bdf9fc", marginBottom:4 }}>Request payout</div>
                <div style={{ fontSize:13, color:"#6ec9c4", marginBottom:20 }}>{"AED "+u.earn.pending.toFixed(2)+" will be paid to your registered IBAN."}</div>
                <div style={{ marginBottom:16, padding:"14px 16px", borderRadius:10, border:"1px solid rgba(0,228,193,0.16)", background:"rgba(0,228,193,0.07)" }}>
                  <div style={{ fontSize:11, color:"#6ec9c4", fontWeight:700, textTransform:"uppercase", letterSpacing:0.5, marginBottom:6 }}>Full available payout</div>
                  <div style={{ fontSize:24, color:"#bdf9fc", fontWeight:700 }}>{"AED "+u.earn.pending.toFixed(2)}</div>
                </div>
                ...
                <div style={{ fontSize:11, color:"#3a7a74", marginBottom:20, padding:"10px 12px", borderRadius:8, background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.04)" }}>
                ...
                <div style={{ display:"flex", gap:10 }}>
                  ...
                  <button onClick={function(){ setWithdrawOpen(false); }} style={{ padding:"12px 18px", borderRadius:10, border:"1px solid rgba(255,255,255,0.08)", background:"transparent", color:"#6ec9c4", fontSize:14, cursor:"pointer" }}>Cancel</button>
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"1fr 1fr", gap:14, marginBottom:16, alignItems:"stretch" }}>
            <div style={{ background:"#0a0f0d", borderRadius:14, padding:mob?14:22, border:"1px solid rgba(255,255,255,0.06)", boxSizing:"border-box" }}>
              <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 4px", color:"#bdf9fc" }}>Weekly Payout History</h3>
              <p style={{ fontSize:11, color:"#3a7a74", margin:"0 0 16px" }}>Requested and completed payout amounts</p>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart margin={{top:5,right:5,left:mob?-25:-10,bottom:5}} data={(u.payouts||[]).map(function(p){return {date:p.date,amount:p.amount,status:p.status}})}>
                  <CartesianGrid strokeDasharray="4 6" stroke="rgba(255,255,255,0.04)" strokeWidth={0.5} vertical={false} />
                  <XAxis dataKey="date" tick={{fill:"#3a7a74",fontSize:9}} axisLine={false} tickLine={false} />
                  <YAxis tick={{fill:"#3a7a74",fontSize:10}} tickFormatter={function(v){return "AED "+v}} axisLine={false} tickLine={false} />
                  <Tooltip contentStyle={{background:"#0a0f0d",border:"1px solid rgba(0,228,193,0.2)",borderRadius:8,fontSize:12,color:"#bdf9fc"}} labelStyle={{color:"#bdf9fc",marginBottom:4}} itemStyle={{color:"#bdf9fc"}} formatter={function(v){return ["AED "+v.toFixed(2),"Amount"]}} />
            ...
            <div style={{ background:"linear-gradient(180deg, rgba(19,19,21,1) 0%, rgba(14,14,16,1) 100%)", borderRadius:16, padding:mob?14:24, border:"1px solid rgba(0,228,193,0.06)", boxShadow:"0 4px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.04)", boxSizing:"border-box" }}>
              <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 4px", color:"#bdf9fc" }}>Total Paid Over Time</h3>
              <p style={{ fontSize:11, color:"#3a7a74", margin:"0 0 16px" }}>Running total of all money transferred to your bank</p>
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart margin={{top:5,right:5,left:mob?-25:-10,bottom:5}} data={(function(){
                  var running = 0;
          ...
          <div style={{ background:"#0a0f0d", borderRadius:14, padding:mob?14:22, border:"1px solid rgba(255,255,255,0.06)", marginBottom:16, boxSizing:"border-box" }}>
            <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 12px", color:"#bdf9fc" }}>Payout Method & Settings</h3>
            <div style={{ background:"rgba(255,255,255,0.03)", borderRadius:10, padding:14, display:"grid", gridTemplateColumns:mob?"1fr":"1fr 1fr", gap:16, alignItems:"stretch" }}>
              <div><div style={{ display:"inline-block", fontSize:9, fontWeight:700, color:"#3a7a74", marginBottom:5, padding:"2px 8px", borderRadius:4, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.05)", letterSpacing:0.8 }}>IBAN</div><div style={{ fontSize:13, fontWeight:600, fontFamily:"monospace", color:"#bdf9fc" }}>{"****  ****  ****  ****  **** " + u.iban.slice(-3)}</div><div style={{ fontSize:11, fontStyle:"italic", color:"#3a7a74", marginTop:4 }}>{"You can update your IBAN in "}<span onClick={function(){gotoTab("settings")}} style={{ color:"rgb(0,228,193)", cursor:"pointer" }}>Settings</span></div></div>
              <div><div style={{ display:"inline-block", fontSize:9, fontWeight:700, color:"#3a7a74", marginBottom:5, padding:"2px 8px", borderRadius:4, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.05)", letterSpacing:0.8 }}>METHOD</div><div style={{ fontSize:13, fontWeight:600, color:"#bdf9fc" }}>MamoPay - Bank Transfer (Weekly)</div></div>
            </div>
          </div>
          <div style={{ background:"#0a0f0d", borderRadius:14, padding:mob?14:22, border:"1px solid rgba(255,255,255,0.06)", boxSizing:"border-box" }}>
            <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 4px", color:"#bdf9fc" }}>Payout History</h3>
            <p style={{ fontSize:11, color:"#3a7a74", margin:"0 0 14px" }}>Requested payouts are reviewed and processed by the admin team.</p>
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              <thead><tr>{["Date","Amount","Status"].map(function(h){return <th key={h} style={{ padding:"8px 10px", fontSize:10, fontWeight:700, textTransform:"uppercase", color:"#3a7a74", textAlign:"left", borderBottom:"2px solid rgba(255,255,255,0.08)" }}>{h}</th>})}</tr></thead>
              <tbody>{(u.payouts||[]).map(function(p){return <tr key={p.date} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)" }}><td style={{ padding:"10px", fontSize:13, color:"#bdf9fc" }}>{p.date}</td><td style={{ padding:"10px", fontSize:14, fontWeight:500, color:"#bdf9fc" }}>{"AED "+p.amount.toFixed(2)}</td><td style={{ padding:"10px" }}><PayoutStatusBadge s={p.status}/></td></tr>})}</tbody>
            </table>
          </div>
        </div>}
          ...
          {activeLesson ? (
            <div>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                <h2 style={{ fontSize:mob?16:20, fontWeight:700, margin:0, color:"#bdf9fc" }}>{activeLesson.title}</h2>
                {activeLesson.done
                  ? <Badge s="completed" />
              ...
              {activeLesson.video_url ? (
                <div style={{ background:"#0a0f0d", borderRadius:16, overflow:"hidden", marginBottom:20, border:"1px solid rgba(255,255,255,0.06)" }}>
                  <iframe
                ...
                <div style={{ background:"#0a0f0d", borderRadius:16, padding:"48px 24px", marginBottom:20, border:"1px solid rgba(255,255,255,0.06)", textAlign:"center" }}>
                  <div style={{ opacity:0.3, lineHeight:0, marginBottom:12 }}><Ico name="book" size={48} color="#6ec9c4" /></div>
                  <div style={{ fontSize:14, color:"#6ec9c4" }}>PDF not yet available for this lesson</div>
                </div>
              )}
              {activeLesson.notes && (
                <div style={{ background:"#0a0f0d", borderRadius:12, padding:"16px 20px", border:"1px solid rgba(255,255,255,0.06)", marginBottom:16 }}>
                  <div style={{ fontSize:11, fontWeight:700, color:"#3a7a74", letterSpacing:0.8, marginBottom:8, textTransform:"uppercase" }}>Lesson Notes</div>
                  <p style={{ fontSize:14, color:"#bdf9fc", lineHeight:1.7, margin:0, whiteSpace:"pre-wrap" }}>{activeLesson.notes}</p>
                </div>
              )}
            </div>
          ) : (
            <div>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
                <h2 style={{ fontSize:22, fontWeight:700, margin:0, color:"#bdf9fc" }}>My Courses</h2>
                <div style={{ background:"#0a0f0d", borderRadius:10, padding:"10px 16px", border:"1px solid rgba(255,255,255,0.06)", display:"flex", alignItems:"center", gap:12 }}>
              ...
              {u.status !== "active" ? (
                <div style={{ background:"#0a0f0d", borderRadius:16, padding:"48px 24px", border:"1px solid rgba(0,228,193,0.15)", textAlign:"center", marginTop:8 }}>
                  <div style={{ lineHeight:0, marginBottom:16 }}><Ico name="lock" size={40} color="rgb(0,228,193)" /></div>
                  <h3 style={{ fontSize:18, fontWeight:700, color:"#bdf9fc", margin:"0 0 8px" }}>Course Access Locked</h3>
                  <p style={{ fontSize:14, color:"#6ec9c4", lineHeight:1.7, maxWidth:380, margin:"0 auto 24px" }}>An active subscription is required to access course materials. Subscribe now to unlock all modules and lessons.</p>
                  <Btn onClick={function(){go("subscribe")}} style={{ fontSize:14, padding:"12px 32px" }}>{"Subscribe · AED "+PRICE+"/month"}</Btn>
                  <div style={{ fontSize:12, color:"#3a7a74", marginTop:12 }}>Already paid? Contact support and we'll activate your account.</div>
                </div>
              ) : courses.map(function(c){
                  ...
                  <div key={c.id} style={{ background:"#0a0f0d", borderRadius:14, border:"1px solid rgba(255,255,255,0.06)", overflow:"hidden", marginBottom:12 }}>
                    <div onClick={async function(){
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:12, color:"#3a7a74" }}>{c.lessons.length > 0 ? done+"/"+c.lessons.length+" completed" : "Click to load lessons"}</div>
                      </div>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#3a7a74" strokeWidth="2.5" strokeLinecap="round"><path d={openCourse===c.id?"M18 15l-6-6-6 6":"M6 9l6 6 6-6"}/></svg>
                    </div>
                    {openCourse===c.id && (
                      <div style={{ borderTop:"1px solid rgba(255,255,255,0.06)" }}>
                        {c.lessons.length === 0 && (
                          <div style={{ padding:"20px", textAlign:"center", color:"#3a7a74", fontSize:13 }}>Loading lessons...</div>
                        )}
                        {c.lessons.map(function(l){ return (
                          <div key={l.id} onClick={function(){setActiveLesson(l)}} style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 20px 12px 60px", cursor:"pointer", borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                            <div style={{ width:20, height:20, borderRadius:"50%", border:"2px solid "+(l.done?"rgb(0,228,193)":"#3a7a74"), background:l.done?"rgb(0,228,193)":"transparent", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, color:"#fff", flexShrink:0 }}>{l.done ? "✓" : ""}</div>
                            <div style={{ flex:1 }}>
                              <div style={{ fontSize:13, fontWeight:500, color:l.done?"#6ec9c4":"#bdf9fc" }}>{l.title}</div>
                              <div style={{ display:"flex", gap:6, marginTop:2 }}>
                                <span style={{ fontSize:10, color:"#3a7a74" }}>{l.dur}</span>
                                {l.video_url ? <span style={{ fontSize:10, color:"rgb(0,228,193)" }}>PDF ✓</span> : <span style={{ fontSize:10, color:"#3a7a74" }}>Coming soon</span>}
                              </div>
                            </div>
                          </div>
                        )})}
        ...
        {tab === "support" && <div>
          {viewTicket ? (
            <div>
              ...
              <div style={{ background:"#0a0f0d", borderRadius:14, border:"1px solid rgba(255,255,255,0.06)", overflow:"hidden", marginBottom:16 }}>
                <div style={{ padding:mob?"14px":"20px 24px", borderBottom:"1px solid rgba(255,255,255,0.06)", display:"flex", justifyContent:"space-between", alignItems:mob?"flex-start":"center", flexDirection:mob?"column":"row", gap:mob?8:0 }}>
                  <div>
                    <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
                      <code style={{ fontSize:10, color:"#3a7a74", background:"#000000", padding:"2px 8px", borderRadius:4 }}>{viewTicket.ref}</code>
                      <Badge s={viewTicket.status} />
                    </div>
                    <h3 style={{ fontSize:mob?16:18, fontWeight:700, color:"#bdf9fc", margin:0 }}>{viewTicket.subject}</h3>
                    <div style={{ fontSize:11, color:"#3a7a74", marginTop:4 }}>{"Created "+viewTicket.created+" \u00B7 "+viewTicket.category}</div>
                  </div>
                </div>
                        ...
                        <div key={i} style={{ display:"flex", gap:12, marginBottom:16, flexDirection:isUser?"row-reverse":"row" }}>
                          ...
                          <div style={{ maxWidth:"75%" }}>
                            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4, flexDirection:isUser?"row-reverse":"row" }}>
                              <span style={{ fontSize:12, fontWeight:600, color:isUser?"#bdf9fc":"#3b82f6" }}>{msg.sender}</span>
                              <span style={{ fontSize:10, color:"#3a7a74" }}>{msg.time}</span>
                            </div>
                            <div style={{ padding:"12px 16px", borderRadius:12, background:isUser?"rgba(0,228,193,0.08)":"rgba(255,255,255,0.03)", border:"1px solid "+(isUser?"rgba(0,228,193,0.12)":"rgba(255,255,255,0.06)"), fontSize:13, color:"#bdf9fc", lineHeight:1.7, borderBottomRightRadius:isUser?4:12, borderBottomLeftRadius:isUser?12:4 }}>{msg.content}</div>
                          </div>
                        </div>
                      );
                    });
            ...
            <div>
              ...
              <h2 style={{ fontSize:mob?18:22, fontWeight:700, margin:"0 0 20px", color:"#bdf9fc" }}>Submit a Support Ticket</h2>

              <div style={{ background:"#0a0f0d", borderRadius:14, padding:mob?16:24, border:"1px solid rgba(255,255,255,0.06)" }}>
                <div style={{ marginBottom:18 }}>
                  <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#6ec9c4", marginBottom:6 }}>Category</label>
                ...
                <div style={{ marginBottom:24 }}>
                  <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#6ec9c4", marginBottom:6 }}>Message</label>
                ...
                <div style={{ marginTop:16, padding:14, background:"rgba(255,255,255,0.02)", borderRadius:10, border:"1px solid rgba(255,255,255,0.04)" }}>
                  <p style={{ fontSize:12, color:"#3a7a74", margin:0 }}>You can also email us directly at <strong style={{ color:"rgb(0,228,193)" }}>support@tutorii.com</strong>. Our team responds within 24 hours (Sun-Thu, 9AM-6PM GST).</p>
                </div>
              </div>
            </div>
          ) : (
            <div>
              <div style={{ display:"flex", flexDirection:mob?"column":"row", justifyContent:"space-between", alignItems:mob?"flex-start":"center", gap:mob?10:0, marginBottom:20 }}>
                <div>
                  <h2 style={{ fontSize:mob?18:22, fontWeight:700, margin:0, color:"#bdf9fc" }}>Support</h2>
                  <p style={{ fontSize:13, color:"#6ec9c4", marginTop:4 }}>View your tickets or get help</p>
                </div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:mob?"1fr 1fr":"repeat(4, 1fr)", gap:10, marginBottom:20 }}>
                {[
                  ...
                  <div key={a[1]} onClick={a[2]} style={{ background:"#0a0f0d", borderRadius:12, padding:"16px 14px", border:"1px solid rgba(255,255,255,0.06)", cursor:"pointer", textAlign:"center", transition:"border-color 0.2s" }}>
                    <div style={{ marginBottom:8, lineHeight:0, display:"flex", justifyContent:"center" }}><Ico name={a[0]} size={20} color="rgb(0,228,193)" /></div>
                    <div style={{ fontSize:11, fontWeight:600, color:"#6ec9c4" }}>{a[1]}</div>
                  </div>
                )})}
              </div>
              ...
              {/* Ticket list */}
              {tickets.length === 0 ? (
                <div style={{ background:"#0a0f0d", borderRadius:14, padding:48, border:"1px solid rgba(255,255,255,0.06)", textAlign:"center" }}>
                  <div style={{ marginBottom:12, lineHeight:0 }}><Ico name="chat" size={36} color="#27272a" /></div>
                  <div style={{ fontSize:15, fontWeight:600, color:"#3a7a74", marginBottom:4 }}>No tickets yet</div>
                  <div style={{ fontSize:13, color:"#3a7a74" }}>When you submit a support request, it will appear here.</div>
                </div>
              ) : (
                <div style={{ background:"#0a0f0d", borderRadius:14, border:"1px solid rgba(255,255,255,0.06)", overflow:"hidden" }}>
                  {tickets.map(function(t, ti) {
                    var statusColors = { open:"rgb(0,228,193)", in_progress:"#3b82f6", resolved:"#10b981", closed:"#6ec9c4" };
                    return (
                      <div key={t.id} onClick={function(){setViewTicket(t)}} style={{ display:"flex", alignItems:mob?"flex-start":"center", gap:mob?10:16, padding:mob?"14px":"16px 20px", borderBottom:ti<tickets.length-1?"1px solid rgba(255,255,255,0.04)":"none", cursor:"pointer", transition:"background 0.2s" }}>
                        <div style={{ width:8, height:8, borderRadius:"50%", background:statusColors[t.status]||"#3a7a74", flexShrink:0, marginTop:mob?6:0 }} />
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:2 }}>
                            <span style={{ fontSize:14, fontWeight:600, color:"#bdf9fc", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{t.subject}</span>
                          </div>
                          <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
                            <code style={{ fontSize:9, color:"#3a7a74", background:"#000000", padding:"1px 6px", borderRadius:3 }}>{t.ref}</code>
                            <span style={{ fontSize:11, color:"#3a7a74" }}>{t.category}</span>
                            <span style={{ fontSize:10, color:"#3a7a74" }}>{"\u00B7"}</span>
                            <span style={{ fontSize:11, color:"#3a7a74" }}>{t.created}</span>
                          </div>
                        </div>
                        <div style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
                          <Badge s={t.status.replace("_"," ")} />
                          <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                            <Ico name="chat" size={12} color="#3a7a74" />
                            <span style={{ fontSize:11, color:"#3a7a74" }}>{t.messages}</span>
                          </div>
                        </div>
                      </div>
                    );
              ...
              <div style={{ marginTop:16, padding:14, background:"rgba(255,255,255,0.02)", borderRadius:10, border:"1px solid rgba(255,255,255,0.04)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <span style={{ fontSize:12, color:"#3a7a74" }}>Our team responds within 24 hours (Sun-Thu, 9AM-6PM GST)</span>
      ...
      {showCancel && !subscriptionCancelled && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center" }} onClick={function(){setShowCancel(false)}}>
          <div onClick={function(e){e.stopPropagation()}} style={{ background:"#0a0f0d", borderRadius:16, padding:32, maxWidth:440, width:"90%", border:"1px solid rgba(248,113,113,0.2)" }}>
            <div style={{ marginBottom:16, lineHeight:0 }}><Ico name="shield" size={36} color="#f87171" /></div>
            <h3 style={{ fontSize:18, fontWeight:700, color:"#bdf9fc", margin:"0 0 8px" }}>Cancel Your Subscription?</h3>
            <p style={{ fontSize:13, color:"#6ec9c4", lineHeight:1.7, marginBottom:8 }}>If you cancel:</p>
            <div style={{ fontSize:13, color:"#6ec9c4", lineHeight:1.9, marginBottom:20, paddingLeft:16 }}>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={function(){setShowCancel(false)}} style={{ flex:1, padding:"11px", borderRadius:10, border:"1px solid rgba(255,255,255,0.1)", background:"transparent", color:"#bdf9fc", fontSize:13, fontWeight:600, cursor:"pointer" }}>Keep Subscription</button>
      ...
      {showOnboard && !dashLoading && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", zIndex:400, display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#0a0f0d", borderRadius:20, padding:"36px 40px", maxWidth:480, width:"90%", textAlign:"center", boxShadow:"0 20px 60px rgba(0,0,0,0.3)" }}>
            {onboard === 0 && <div>
              <div style={{ marginBottom:16, lineHeight:0 }}><Ico name="sparkle" size={48} color="rgb(0,228,193)" /></div>
              <h2 style={{ fontSize:20, fontWeight:500, color:"#bdf9fc", margin:"0 0 8px" }}>{"Welcome to Tutorii, "+(u.name||"there").split(" ")[0]+"!"}</h2>
              <p style={{ fontSize:14, color:"#6ec9c4", lineHeight:1.7, marginBottom:24 }}>{"Let's take a quick tour of your dashboard so you can start learning and earning right away."}</p>
            </div>}
            {onboard === 1 && <div>
              <div style={{ marginBottom:16, lineHeight:0 }}><Ico name="link" size={48} color="rgb(0,228,193)" /></div>
              <h2 style={{ fontSize:20, fontWeight:700, color:"#bdf9fc", margin:"0 0 8px" }}>Your Referral Link</h2>
              ...
              <p style={{ fontSize:14, color:"#6ec9c4", lineHeight:1.7, marginBottom:16 }}>{"Share this link with friends and contacts. When they subscribe, you earn 40% (AED "+(PRICE*L1_RATE).toFixed(2)+") every month. Use the WhatsApp, SMS, and Email buttons at the top of your dashboard to share instantly."}</p>
            </div>}
            {onboard === 2 && <div>
              <div style={{ marginBottom:16, lineHeight:0 }}><Ico name="book" size={48} color="rgb(0,228,193)" /></div>
              <h2 style={{ fontSize:20, fontWeight:700, color:"#bdf9fc", margin:"0 0 8px" }}>Start Learning</h2>
              <p style={{ fontSize:14, color:"#6ec9c4", lineHeight:1.7, marginBottom:16 }}>{"You have access to "+courses.length+" modules with "+courses.reduce(function(s,c){return s+c.lessons.length},0)+" lessons. Head to the Courses tab to start watching videos and downloading guides. Mark lessons complete to track your progress."}</p>
            </div>}
            {onboard === 3 && <div>
              <div style={{ marginBottom:16, lineHeight:0 }}><Ico name="dollar" size={48} color="rgb(0,228,193)" /></div>
              <h2 style={{ fontSize:20, fontWeight:700, color:"#bdf9fc", margin:"0 0 8px" }}>Track Your Earnings</h2>
              <p style={{ fontSize:14, color:"#6ec9c4", lineHeight:1.7, marginBottom:16 }}>{"The Earnings and Payouts tabs show your income in real time. Payouts are processed every Tuesday via MamoPay to your bank account. Make sure your IBAN is set up in Settings."}</p>
            </div>}
            {onboard === 4 && <div>
              <div style={{ marginBottom:16, lineHeight:0 }}><Ico name="chat" size={48} color="rgb(0,228,193)" /></div>
              <h2 style={{ fontSize:20, fontWeight:700, color:"#bdf9fc", margin:"0 0 8px" }}>Need Help? Ask the Bot</h2>
              <p style={{ fontSize:14, color:"#6ec9c4", lineHeight:1.7, marginBottom:16 }}>{"Click the chat icon in the bottom-right corner anytime. Our AI assistant knows your account details and can answer questions about your earnings, referrals, courses, and more."}</p>
            </div>}
            ...
            <div style={{ display:"flex", gap:10, justifyContent:"center" }}>
              {onboard > 0 && <button onClick={function(){setOnboard(onboard-1)}} style={{ padding:"11px 24px", borderRadius:10, border:"1px solid rgba(255,255,255,0.1)", background:"#0a0f0d", fontSize:13, fontWeight:600, color:"#6ec9c4", cursor:"pointer" }}>Back</button>}
              {onboard < 4 ? (
            ...
            {onboard < 4 && <div style={{ marginTop:12 }}><span onClick={function(){localStorage.setItem("tutorii_tour_done","1");setShowOnboard(false)}} style={{ fontSize:12, color:"#3a7a74", cursor:"pointer" }}>Skip tour</span></div>}
          </div>
        </div>
      )}
      ...
      {(!chatOpen || chatMinimized) && (
        <button onClick={function(){setChatOpen(true);setChatMinimized(false)}} style={{
          ...
          boxShadow:"0 4px 24px rgba(0,228,193,0.15), 0 8px 40px rgba(0,0,0,0.3)", display:"flex", alignItems:"center", justifyContent:"center", gap:8,
      ...
      {chatOpen && !chatMinimized && (
        <div style={{
          ...
          background:"#0a0f0d", borderRadius:16, boxShadow:"0 8px 40px rgba(0,0,0,0.2)",
          display:"flex", flexDirection:"column", overflow:"hidden", zIndex:300,
          border:"1px solid rgba(255,255,255,0.08)"
        }}>
          {/* Chat Header */}
          <div style={{ padding:"16px 20px", background:"linear-gradient(135deg, #0a0f0d, #18181b)", borderBottom:"1px solid rgba(255,255,255,0.06)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <div style={{ width:32, height:32, borderRadius:"50%", background:"rgba(0,228,193,0.2)", border:"1px solid rgba(0,228,193,0.15)", display:"flex", alignItems:"center", justifyContent:"center", lineHeight:0 }}><Ico name="bot" size={16} color="rgb(0,228,193)" /></div>
              <div>
            ...
            {chatLoading && (
              <div style={{ display:"flex", justifyContent:"flex-start" }}>
                <div style={{ padding:"10px 14px", borderRadius:12, background:"rgba(255,255,255,0.04)", borderBottomLeftRadius:4 }}>
                  <div style={{ display:"flex", gap:4 }}>
                    <div style={{ width:6, height:6, borderRadius:"50%", background:"#3a7a74", animation:"pulse 1s infinite" }} />
                    <div style={{ width:6, height:6, borderRadius:"50%", background:"#3a7a74", animation:"pulse 1s infinite 0.2s" }} />
                    <div style={{ width:6, height:6, borderRadius:"50%", background:"#3a7a74", animation:"pulse 1s infinite 0.4s" }} />
                  </div>
                </div>
              </div>
            )}
          ...
          {chatMsgs.length <= 1 && (
            <div style={{ padding:"0 16px 8px" }}>
              <div style={{ fontSize:9, fontWeight:700, color:"#3a7a74", letterSpacing:0.5, marginBottom:6, textTransform:"uppercase" }}>Ask me anything</div>
                ...
                {["How much have I earned?","Who are my referrals?","My course progress","How do payouts work?"].map(function(q){
                  return (
                    <button key={q} onClick={function(){
                      ...
                      padding:"5px 10px", borderRadius:16, border:"1px solid rgba(255,255,255,0.08)", background:"#0a0f0d",
            ...
            <div style={{ padding:"4px 16px 6px", borderTop:"1px solid rgba(255,255,255,0.04)" }}>
              <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
                {[
                  ...
                  <button key={a[0]} onClick={a[2]} style={{ display:"flex", alignItems:"center", gap:4, padding:"4px 9px", borderRadius:12, border:"1px solid rgba(255,255,255,0.06)", background:"rgba(255,255,255,0.02)", fontSize:9, fontWeight:600, color:"#6ec9c4", cursor:"pointer" }}>
                    <Ico name={a[1]} size={10} color="#3a7a74" />{a[0]}
                  </button>
                )})}
              </div>
            </div>
          ...
          <div style={{ padding:"6px 16px", borderTop:"1px solid rgba(255,255,255,0.04)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            ...
            }} style={{ fontSize:10, color:"#3a7a74", cursor:"pointer", display:"flex", alignItems:"center", gap:4 }}>
              <Ico name="users" size={10} color="#3a7a74" />
              <span>{"Need a human? "}<span style={{ color:"rgb(0,228,193)", fontWeight:600 }}>Escalate to support</span></span>
            </span>
            <span onClick={function(){gotoTab("support");setChatOpen(false)}} style={{ fontSize:10, color:"#3a7a74", cursor:"pointer" }}>View tickets</span>
          </div>
          ...
          <div style={{ padding:"10px 16px", borderTop:"1px solid rgba(255,255,255,0.06)", display:"flex", gap:8, alignItems:"center" }}>
            <input
              ...
              style={{ flex:1, padding:"10px 14px", borderRadius:10, border:"1px solid rgba(255,255,255,0.08)", fontSize:13, outline:"none", background:"#000000", color:"#bdf9fc", fontFamily:"'Plus Jakarta Sans',sans-serif" }}
            />
