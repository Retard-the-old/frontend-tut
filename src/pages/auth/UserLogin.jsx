  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:mob?"column":"row", background:"#000000" }}>
      <div style={{ flex:mob?"none":"0 0 50%", background:"#080c0b", display:"flex", flexDirection:"column", justifyContent:"center", padding:mob?"32px 24px":"64px 56px" }}>
        ...
        <p style={{ fontSize:mob?14:15, color:"rgb(0,228,193)", lineHeight:1.7, maxWidth:380 }}>Log in to access your courses, track your referrals, and manage your earnings.</p>
      </div>
      <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:mob?"24px 20px":48 }}>
        <div style={{ maxWidth:400, width:"100%" }}>
          <h3 style={{ fontSize:24, fontWeight:700, color:"#bdf9fc", margin:"0 0 8px" }}>Log in to your account</h3>
          <p style={{ fontSize:14, color:"#6ec9c4", marginBottom:32 }}>Enter your email and password</p>
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <input type="email" value={email} onChange={function(e){setEmail(e.target.value);setError("")}} placeholder="you@example.com" style={{ width:"100%", padding:"12px 16px", borderRadius:10, border:"1px solid "+(error ? "rgba(248,113,113,0.3)" : "rgba(255,255,255,0.1)"), fontSize:14, outline:"none", boxSizing:"border-box", fontFamily:"'Plus Jakarta Sans',sans-serif", background:"#000000", color:"#bdf9fc", transition:"border-color 0.3s" }} />
          </div>
          <div style={{ marginBottom:8 }}>
            <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#6ec9c4", marginBottom:6 }}>Password</label>
          ...
          <div style={{ textAlign:"right", marginBottom:24 }}>
            <span onClick={function(){go("forgotPassword")}} style={{ fontSize:12, color:"rgb(0,228,193)", cursor:"pointer", fontWeight:500 }}>Forgot password?</span>
          </div>
          ...
          <div style={{ textAlign:"center", fontSize:13, color:"#6ec9c4" }}>
            {"No account? "}<span onClick={function(){go("subscribe")}} style={{ color:"rgb(0,228,193)", fontWeight:600, cursor:"pointer" }}>Subscribe now</span>
          </div>
          <div style={{ textAlign:"center", marginTop:16 }}>
            <span onClick={function(){go("landing")}} style={{ fontSize:12, color:"#3a7a74", cursor:"pointer" }}>Back to home</span>
          </div>
        </div>
      </div>
    </div>
