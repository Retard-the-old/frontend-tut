    ...
    <div style={{ background:"#000000", color:"#bdf9fc", position:"relative" }}>
      ...
      <section style={{ paddingTop:mob?120:160, padding:mob?"120px 20px 60px":"160px 48px 100px", textAlign:"center", position:"relative", overflow:"hidden" }}>
        ...
        <FadeIn><div style={{ maxWidth:700, margin:"0 auto", position:"relative", zIndex:1 }}>
          ...
          <h1 style={{ fontSize:mob?32:48, fontWeight:600, color:"#bdf9fc", margin:"0 0 18px", lineHeight:1.1, letterSpacing:mob?"-1px":"-1.5px" }}>Education for <ShimmerText style={{ fontStyle:"italic", fontFamily:"Georgia, serif", fontWeight:200, fontSize:48, letterSpacing:"-1.5px" }}>Your Life Here</ShimmerText></h1>
          <p style={{ fontSize:16, color:"#6ec9c4", maxWidth:560, margin:"0 auto 32px", lineHeight:1.7 }}>Practical skills for expatriates. Every lesson built for the Gulf.</p>
          <div style={{ display:"flex", gap:48, justifyContent:"center" }}>
            {[[INIT_COURSES.length+"","Modules"],[tl+"","Lessons"]].map(function(s){return <div key={s[1]}><div style={{ fontSize:32, fontWeight:600, color:"#bdf9fc", letterSpacing:"-1px" }}>{s[0]}</div><div style={{ fontSize:12, color:"#3a7a74", marginTop:2 }}>{s[1]}</div></div>})}
          </div>
        </div></FadeIn>
      </section>
          ...
          {INIT_COURSES.map(function(c,ci){return (
            <FadeIn key={c.id} delay={ci*0.1}><SpotlightCard padding={32} outerStyle={{ marginBottom:16 }}>
              <div style={{ display:mob?"flex":"flex", flexDirection:mob?"column":"row", alignItems:mob?"flex-start":"center", gap:mob?12:16, marginBottom:18 }}>
                ...
                <div style={{ flex:1 }}><div style={{ fontSize:12, fontWeight:500, color:"#3a7a74", letterSpacing:"1px" }}>{"MODULE "+(ci+1)}</div><h2 style={{ fontSize:mob?18:20, fontWeight:600, color:"#bdf9fc", margin:"2px 0 6px" }}>{c.module}</h2><p style={{ fontSize:13, color:"#3a7a74", margin:0 }}>{md[c.module]||""}</p></div>
                {!mob && <div style={{ fontSize:12, color:"#3a7a74" }}>{c.lessons.length+" lessons"}</div>}
              </div>
              {c.lessons.map(function(l,li){return (
                <div key={l.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"13px 0", borderTop:"1px solid rgba(255,255,255,0.04)", minHeight:44 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                    <div style={{ width:26, height:26, borderRadius:7, background:"rgba(255,255,255,0.03)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:500, color:"#3a7a74", flexShrink:0, lineHeight:1 }}>{li+1}</div>
                    <span style={{ fontSize:14, color:"#bdf9fc" }}>{l.title}</span>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <span style={{ fontSize:12, color:"#3a7a74" }}>{l.dur}</span>
