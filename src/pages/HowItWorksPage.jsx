    ...
    <div style={{ background:"#000000", color:"#ffffff", position:"relative" }}>
      ...
      <section style={{ paddingTop:160, padding:"160px 48px 100px", textAlign:"center", position:"relative", overflow:"hidden" }}>
        ...
        <FadeIn><div style={{ maxWidth:700, margin:"0 auto", position:"relative", zIndex:1 }}>
          ...
          <h1 style={{ fontSize:52, fontWeight:600, color:"#ffffff", margin:"0 0 18px", lineHeight:1.1, letterSpacing:"-1.5px" }}>A Simple Model That <ShimmerText style={{ fontStyle:"italic", fontFamily:"Georgia, serif", fontWeight:200, fontSize:52, letterSpacing:"-1.5px" }}>Rewards You</ShimmerText></h1>
          <p style={{ fontSize:16, color:"#94a3b8", maxWidth:560, margin:"0 auto", lineHeight:1.7 }}>Tutorii gives you access to expert-led courses and a built-in way to earn recurring income.</p>
        </div></FadeIn>
      </section>
        ...
        <div style={{ maxWidth:760, margin:"0 auto", position:"relative" }}>
          ...
          {[{n:"01",i:"phone",t:"Subscribe and get immediate access",d:"Pay AED "+PRICE+"/month via MamoPay (Visa, Mastercard, Apple Pay, Google Pay). Unlock every course, every lesson, and your referral dashboard instantly."},{n:"02",i:"book",t:"Learn skills that matter",d:INIT_COURSES.length+" modules, "+tl+" lessons covering UAE culture, legal rights, career development, financial literacy, and entrepreneurship."},{n:"03",i:"link",t:"Share your personal referral link",d:"Every subscriber gets a unique link. Share through WhatsApp, SMS, email, or social media. When someone subscribes, they are permanently connected to you."},{n:"04",i:"dollar",t:"Earn recurring commissions weekly",d:"40% (AED "+(PRICE*L1_RATE).toFixed(2)+") per direct referral per month. 5% (AED "+(PRICE*L2_RATE).toFixed(2)+") on their referrals. Paid every Tuesday to your bank."}].map(function(s,si){return (
            <FadeIn key={s.n} delay={si*0.15}><div style={{ display:"flex", gap:24, marginBottom:48, position:"relative", zIndex:1 }}>
              ...
              <div><div style={{ fontSize:12, fontWeight:500, color:"#64748b", letterSpacing:"1px", marginBottom:6 }}>{"STEP "+s.n}</div><h3 style={{ fontSize:18, fontWeight:600, color:"#ffffff", margin:"0 0 8px" }}>{s.t}</h3><p style={{ fontSize:14, color:"#94a3b8", lineHeight:1.8, margin:0 }}>{s.d}</p></div>
            </div></FadeIn>
          )})}
      ...
      <section style={{ padding:"140px 48px", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ maxWidth:1100, margin:"0 auto", position:"relative", zIndex:1 }}>
          <FadeIn><div style={{ textAlign:"center", marginBottom:64 }}><SLabel icon="dollar">Earning Potential</SLabel><h2 style={{ fontSize:42, fontWeight:600, color:"#ffffff", margin:"0 0 14px", letterSpacing:"-1px" }}>Two Ways to Earn. Both <span style={{ fontStyle:"italic", fontFamily:"Georgia, serif", fontWeight:200, color:"rgb(0,228,193)" }}>Recurring.</span></h2><p style={{ fontSize:15, color:"#94a3b8", maxWidth:480, margin:"0 auto" }}>Tutorii pays you every month for as long as your referrals stay subscribed.</p></div></FadeIn>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, maxWidth:720, margin:"0 auto 40px", alignItems:"stretch" }}>
            <FadeIn delay={0.1}><SpotlightCard padding={40} style={{ textAlign:"center", justifyContent:"center", alignItems:"center" }} outerStyle={{ background:"linear-gradient(180deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.03) 100%)" }}><div style={{ fontSize:12, fontWeight:500, color:"#ffffff", letterSpacing:"1px", marginBottom:12, minHeight:32, display:"flex", alignItems:"center", justifyContent:"center" }}>LEVEL 1 · DIRECT</div><div style={{ fontSize:40, fontWeight:500, color:"#ffffff", letterSpacing:"-1.5px" }}>40%</div><div style={{ fontSize:16, fontWeight:400, color:"#ffffff", margin:"4px 0 16px" }}>{"AED "+(PRICE*L1_RATE).toFixed(2)+"/mo each"}</div><p style={{ fontSize:13, color:"#94a3b8", lineHeight:1.7, margin:0 }}>For every person who subscribes through your link.</p></SpotlightCard></FadeIn>
            <FadeIn delay={0.2}><SpotlightCard padding={40} style={{ textAlign:"center", justifyContent:"center", alignItems:"center" }}><div style={{ fontSize:12, fontWeight:500, color:"#94a3b8", letterSpacing:"1px", marginBottom:12, minHeight:32, display:"flex", alignItems:"center", justifyContent:"center" }}>LEVEL 2 · INDIRECT</div><div style={{ fontSize:40, fontWeight:500, color:"#ffffff", letterSpacing:"-1.5px" }}>5%</div><div style={{ fontSize:16, fontWeight:400, color:"#94a3b8", margin:"4px 0 16px" }}>{"AED "+(PRICE*L2_RATE).toFixed(2)+"/mo each"}</div><p style={{ fontSize:13, color:"#64748b", lineHeight:1.7, margin:0 }}>For subscribers brought in by your referrals.</p></SpotlightCard></FadeIn>
          </div>
          <FadeIn delay={0.3}><GCard padding={24} outerStyle={{ maxWidth:720, margin:"0 auto" }}><div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>{["No cap on referrals","No recruitment requirement","Paid every Tuesday to your bank","Only 2 levels  - not MLM","No inventory or quotas","Earn from day one"].map(function(f){return <div key={f} style={{ fontSize:13, color:"#94a3b8", display:"flex", gap:8, alignItems:"center", lineHeight:1.5 }}><span style={{ color:"#ffffff" }}>{"\u2713"}</span>{f}</div>})}</div></GCard></FadeIn>
        </div>
      </section>
      ...
      <section style={{ padding:"140px 48px", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ maxWidth:580, margin:"0 auto", textAlign:"center", position:"relative", zIndex:1 }}>
          <FadeIn><h2 style={{ fontSize:32, fontWeight:600, color:"#ffffff", margin:"0 0 8px", letterSpacing:"-0.5px" }}>Income <span style={{ fontStyle:"italic", fontFamily:"Georgia, serif", fontWeight:200, color:"rgb(0,228,193)" }}>Calculator</span></h2><p style={{ fontSize:14, color:"#94a3b8", marginBottom:40 }}>Drag the slider to see your potential monthly earnings</p></FadeIn>
          <FadeIn delay={0.15}><GCard padding={40}>
            <div style={{ marginBottom:28 }}>
              <input type="range" min={1} max={100} value={refs} onChange={function(e){setRefs(Number(e.target.value))}} style={{ width:"100%", accentColor:"#ffffff", height:2 }} />
              <div style={{ fontSize:40, fontWeight:600, color:"#ffffff", marginTop:12, letterSpacing:"-1px" }}>{refs+" referral"+(refs===1?"":"s")}</div>
              <div style={{ fontSize:12, color:"#64748b" }}>{"+ ~"+estL2+" indirect referrals"}</div>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, maxWidth:420, margin:"0 auto" }}>
              {[["Direct 40%",l1earn],["Indirect 5%",l2earn],["Net Profit",net]].map(function(d){return (
                <div key={d[0]} style={{ background: d[0]==="Net Profit" && net>0 ? "rgba(0,228,193,0.06)" : "rgba(255,255,255,0.03)", borderRadius:12, padding:16, border:"1px solid "+(d[0]==="Net Profit" && net>0 ? "rgba(0,228,193,0.12)" : "rgba(255,255,255,0.04)"), textAlign:"center", transition:"background 0.3s, border-color 0.3s" }}>
                  <div style={{ fontSize:11, color:"#64748b", marginBottom:4 }}>{d[0]}</div>
                  <div style={{ fontSize:18, fontWeight:500, color:d[0]==="Net Profit"&&d[1]<0?"#ef4444":"#ffffff" }}>{"AED "+d[1].toFixed(2)}</div>
                </div>
              )})}
            </div>
            <div style={{ marginTop:16, fontSize:12, color:"#64748b" }}>{"AED "+(net*12).toFixed(2)+"/year"}</div>
          </GCard></FadeIn>
      ...
      <section style={{ padding:"80px 48px 140px", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ maxWidth:820, margin:"0 auto", position:"relative", zIndex:1 }}>
          <FadeIn><div style={{ textAlign:"center", marginBottom:40 }}><h2 style={{ fontSize:28, fontWeight:600, color:"#ffffff", letterSpacing:"-0.5px" }}>Realistic Income <span style={{ fontStyle:"italic", fontFamily:"Georgia, serif", fontWeight:200, color:"rgb(0,228,193)" }}>Scenarios</span></h2></div></FadeIn>
