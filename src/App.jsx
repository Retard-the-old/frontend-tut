...
function Landing(props) {
  ...
  return (
    <div style={{ background:"#0a0a0c", color:"#d4d4d8", position:"relative", overflowX:"hidden" }}>
      <NoiseOverlay />
      <SiteNav go={go} />
  ...
  return (
    <div>
      ...
      <div style={{ background:"#131315", borderRadius:14, padding:mob?16:22, border:"1px solid rgba(255,255,255,0.06)", marginBottom:16, boxSizing:"border-box" }}>
        ...
        <p style={{ fontSize:12, color:"#52525b", marginBottom:16 }}>Enter and confirm your new password below.</p>
      ...
      <div ref={contentRef} style={{ flex:1, padding:mob?16:28, overflow:"auto", maxHeight:"100vh", paddingBottom:mob?80:28, overflowX:"hidden", boxSizing:"border-box", minWidth:0, width:mob?"100%":"auto" }}>
        ...
        {tab === "payouts" && <div style={{ maxWidth:"100%", overflow:"hidden" }}>
          ...
          <div style={{ background:"#131315", borderRadius:14, padding:mob?14:22, border:"1px solid rgba(255,255,255,0.06)", marginBottom:16, boxSizing:"border-box" }}>
            ...
            <p style={{ margin:"10px 0 0", fontSize:11, color:"#52525b", fontStyle:"italic" }}>Payments might take up to 5 working days to arrive. Please refer to the <a href="https://www.tutorii.com/terms" target="_blank" rel="noopener noreferrer" style={{ color:"rgb(200,180,140)", textDecoration:"none" }}>terms and conditions</a>.</p>
          </div>
          <div style={{ background:"#131315", borderRadius:14, padding:mob?14:22, border:"1px solid rgba(255,255,255,0.06)", boxSizing:"border-box" }}>
...
function AdminPanel(props) {
  ...
  var _payoutFilter = useState("all"); var payoutFilter = _payoutFilter[0]; var setPayoutFilter = _payoutFilter[1];
  ...
  return (
    <div style={{ display:"flex", flexDirection:mob?"column":"row", minHeight:"100vh", background:"#0a0a0c" }}>
      ...
      <div ref={adminContentRef} style={{ flex:1, padding:mob?16:28, overflow:"auto", maxHeight:"100vh", paddingBottom:mob?80:28, overflowX:"hidden", boxSizing:"border-box", minWidth:0, width:mob?"100%":"auto" }}>
        ...
        {tab === "payouts" && (function(){
          var filteredPayouts = payoutFilter === "all" ? payouts : payouts.filter(function(p){
            if (payoutFilter === "completed") return p.status === "completed";
            if (payoutFilter === "failed") return p.status === "failed";
            if (payoutFilter === "pending") return p.status === "processing" || p.status === "queued" || p.status === "pending";
            return true;
          });
          var failedPayouts = payouts.filter(function(p){return p.status==="failed"});
          var counts = {
            all: payouts.length,
            completed: payouts.filter(function(p){return p.status==="completed"}).length,
            failed: failedPayouts.length,
            pending: payouts.filter(function(p){return p.status==="processing"||p.status==="queued"||p.status==="pending"}).length,
          };
          return <div style={{ maxWidth:"100%", overflow:"hidden" }}>
            {/* Header */}
            <div style={{ display:"flex", flexDirection:mob?"column":"row", justifyContent:"space-between", alignItems:mob?"flex-start":"center", gap:mob?10:0, marginBottom:16 }}>
            ...
            <div style={{ display:"flex", gap:6, marginBottom:12, flexWrap:"wrap" }}>
              {[["all","All"],["completed","Completed"],["failed","Failed"],["pending","Pending"]].map(function(f){
                var active = payoutFilter === f[0];
                var color = f[0]==="completed"?"#10b981":f[0]==="failed"?"#f87171":f[0]==="pending"?"#f59e0b":"#71717a";
                return <button key={f[0]} onClick={function(){setPayoutFilter(f[0])}} style={{ padding:"6px 14px", borderRadius:8, border:"1px solid "+(active?color+"40":"rgba(255,255,255,0.06)"), background:active?color+"15":"transparent", color:active?color:"#52525b", fontSize:11, fontWeight:600, cursor:"pointer" }}>
                  {f[1]}{counts[f[0]] > 0 ? " ("+counts[f[0]]+")" : ""}
                </button>;
              })}
            </div>

            {/* Bulk actions bar — only shown when there are failed payouts */}
            {failedPayouts.length > 0 && <div style={{ display:"flex", gap:8, alignItems:"center", padding:"10px 14px", background:"rgba(248,113,113,0.06)", border:"1px solid rgba(248,113,113,0.15)", borderRadius:10, marginBottom:14, flexWrap:"wrap" }}>
              <span style={{ fontSize:12, color:"#f87171", fontWeight:600, marginRight:4 }}>{failedPayouts.length+" failed payout"+(failedPayouts.length>1?"s":"")}</span>
              <button onClick={async function(){
                try {
                  var result = await adminApi.triggerPayouts();
                  flash("Retried: "+(result.payouts_processed||0)+" processed");
                  var updated = await adminApi.payouts();
                  if (Array.isArray(updated)) setPayouts(updated);
                } catch(err){ flash("Retry failed: "+(err.message||"Unknown error")); }
              }} style={{ padding:"5px 12px", borderRadius:6, border:"1px solid rgba(200,180,140,0.2)", background:"rgba(200,180,140,0.06)", fontSize:11, fontWeight:600, color:"rgb(200,180,140)", cursor:"pointer" }}>Retry All Failed</button>
              <button onClick={async function(){
                if (!window.confirm("Send IBAN error email to "+failedPayouts.length+" user(s)?")) return;
                try {
                  var res = await adminApi.notifyFailedPayouts();
                  flash("Notified "+res.notified+" user(s)");
                } catch(err){ flash("Notify failed: "+(err.message||"Unknown error")); }
              }} style={{ padding:"5px 12px", borderRadius:6, border:"1px solid rgba(248,113,113,0.2)", background:"rgba(248,113,113,0.06)", fontSize:11, fontWeight:600, color:"#f87171", cursor:"pointer" }}>Notify Users to Fix IBAN</button>
            </div>}
            ...
            <div style={{ background:"#131315", borderRadius:14, border:"1px solid rgba(255,255,255,0.06)", overflow:"hidden" }}>
              <div style={{ overflowX:"auto", WebkitOverflowScrolling:"touch" }}><table style={{ width:"100%", borderCollapse:"collapse", minWidth:mob?750:"auto" }}>
                <thead><tr>
                  <th style={{ padding:"12px", width:40, verticalAlign:"middle", borderBottom:"1px solid rgba(255,255,255,0.06)" }}><input type="checkbox" onChange={function(e){setSel(e.target.checked ? filteredPayouts.filter(function(p){return p.status==="queued"}).map(function(p){return p.id}) : [])}} /></th>
                ...
                <tbody>{filteredPayouts.length === 0 ? (
                  <tr><td colSpan={7} style={{ padding:40, textAlign:"center", color:"#3f3f46", fontSize:13 }}>No {payoutFilter !== "all" ? payoutFilter : ""} payouts</td></tr>
                ) : filteredPayouts.map(function(p){ return (
                  <tr key={p.id} style={{ borderBottom:"1px solid rgba(255,255,255,0.06)", background:sel.includes(p.id)?"rgba(200,180,140,0.12)":p.status==="failed"?"rgba(248,113,113,0.03)":"transparent" }}>
                    ...
                    <td style={{ padding:"12px", fontSize:13, fontWeight:600, color:"#d4d4d8", verticalAlign:"middle" }}>{p.user}<div style={{ fontSize:10, color:"#52525b" }}>{p.email||""}</div></td>
                    <td style={{ padding:"12px", verticalAlign:"middle" }}><code style={{ fontSize:11, color:"#52525b" }}>{p.iban||"—"}</code>{p.iban_name && <div style={{ fontSize:10, color:"#3f3f46", marginTop:2 }}>{p.iban_name}</div>}</td>
                    <td style={{ padding:"12px", fontSize:14, fontWeight:500, color:"#d4d4d8", verticalAlign:"middle" }}>{"AED "+p.amount.toFixed(2)}</td>
                    <td style={{ padding:"12px", verticalAlign:"middle" }}><Badge s={p.status}/>{p.status==="failed" && p.failure_reason && <div style={{ fontSize:9, color:"#f87171", marginTop:3, maxWidth:160, wordBreak:"break-word" }}>{p.failure_reason.slice(0,80)}</div>}</td>
                    <td style={{ padding:"12px", fontSize:12, color:"#71717a", verticalAlign:"middle" }}>{p.date}</td>
                    <td style={{ padding:"12px", verticalAlign:"middle" }}>
                      <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
                        {p.status === "failed" && <button onClick={async function(e){e.stopPropagation(); try { var result = await adminApi.triggerPayouts(); flash("Retried: "+(result.payouts_processed||0)+" processed"); var updated = await adminApi.payouts(); if(Array.isArray(updated)) setPayouts(updated); } catch(err){ flash("Retry failed: "+(err.message||"Unknown")); }}} style={{ padding:"5px 10px", borderRadius:6, border:"1px solid rgba(200,180,140,0.2)", background:"rgba(200,180,140,0.06)", fontSize:10, fontWeight:600, color:"rgb(200,180,140)", cursor:"pointer", whiteSpace:"nowrap" }}>Retry</button>}
                        {(p.status === "failed" || p.status === "processing") && <button onClick={function(e){e.stopPropagation(); setPayouts(function(prev){return prev.map(function(x){return x.id===p.id?Object.assign({},x,{status:"completed"}):x})}); flash("Payout marked as completed")}} style={{ padding:"5px 10px", borderRadius:6, border:"1px solid rgba(16,185,129,0.2)", background:"rgba(16,185,129,0.06)", fontSize:10, fontWeight:600, color:"#10b981", cursor:"pointer", whiteSpace:"nowrap" }}>Mark Done</button>}
