                ...
                <tr key={p.id} style={{ borderBottom:"1px solid rgba(255,255,255,0.06)", background:sel.includes(p.id)?"rgba(200,180,140,0.12)":"transparent" }}>
                  ...
                  <td style={{ padding:"12px", verticalAlign:"middle" }}>
                    <div style={{ display:"flex", gap:4 }}>
                      ...
                      {p.status === "completed" && !p.mamopay_transfer_id && <button onClick={async function(e){e.stopPropagation(); if (!window.confirm("Reset this payout? Jiya's commissions will return to pending so the payout can be re-processed.")) return; try { await admin.resetPayout(p.id); setPayouts(function(prev){ return prev.map(function(x){ return x.id===p.id ? Object.assign({},x,{status:"failed"}) : x }) }); flash("Payout reset — commissions returned to pending"); } catch(err) { flash("Reset failed: "+(err.message||"unknown error")); }}} style={{ padding:"5px 10px", borderRadius:6, border:"1px solid rgba(239,68,68,0.2)", background:"rgba(239,68,68,0.06)", fontSize:10, fontWeight:600, color:"#f87171", cursor:"pointer", whiteSpace:"nowrap" }}>Reset</button>}
