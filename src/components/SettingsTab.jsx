...
function SettingsTab(props) {
  ...
  var inputStyle = { width:"100%", padding:"11px 14px", borderRadius:10, border:"1px solid rgba(255,255,255,0.1)", fontSize:14, outline:"none", boxSizing:"border-box", fontFamily:"'Plus Jakarta Sans',sans-serif", background:"#000000", color:"#ffffff" };

  return (
    <div>
      <h2 style={{ fontSize:mob?18:22, fontWeight:700, margin:"0 0 20px", color:"#ffffff" }}>Account Settings</h2>

      {/* Profile */}
      <div style={{ background:"#0d0d0d", borderRadius:14, padding:mob?16:22, border:"1px solid rgba(255,255,255,0.06)", marginBottom:16, boxSizing:"border-box" }}>
        <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 16px", color:"#ffffff" }}>Profile Information</h3>
        ...
        <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"1fr 1fr", gap:mob?12:16 }}>
          <div>
            <label style={{ display:"block", fontSize:11, fontWeight:600, color:"#64748b", marginBottom:4 }}>Full Name</label>
            <input value={name} onChange={function(e){setName(e.target.value);setProfileMsg(null)}} style={inputStyle} />
          </div>
          <div>
            <label style={{ display:"block", fontSize:11, fontWeight:600, color:"#64748b", marginBottom:4 }}>Email Address</label>
            <input value={u.email} disabled style={Object.assign({},inputStyle,{opacity:0.5,cursor:"not-allowed"})} />
          </div>
          <div>
            <label style={{ display:"block", fontSize:11, fontWeight:600, color:"#64748b", marginBottom:4 }}>Referral Code</label>
            <div style={{ padding:"11px 14px", borderRadius:10, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)", fontSize:14, fontWeight:600, fontFamily:"monospace", color:"#ffffff" }}>{u.code}</div>
          </div>
          <div>
            <label style={{ display:"block", fontSize:11, fontWeight:600, color:"#64748b", marginBottom:4 }}>Member Since</label>
            <div style={{ padding:"11px 14px", borderRadius:10, background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)", fontSize:14, color:"#ffffff" }}>{u.joined || "—"}</div>
          </div>
        </div>
      ...
      {/* Payout Details */}
      <div style={{ background:"#0d0d0d", borderRadius:14, padding:mob?16:22, border:"1px solid rgba(255,255,255,0.06)", marginBottom:16, boxSizing:"border-box" }}>
        <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 6px", color:"#ffffff" }}>Payout Details</h3>
        <p style={{ fontSize:12, color:"#64748b", marginBottom:16 }}>Your earnings are paid every Tuesday via MamoPay to your registered IBAN. Minimum payout: AED 50.</p>
        ...
        <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"1fr 1fr", gap:mob?12:16 }}>
          <div>
            <label style={{ display:"block", fontSize:11, fontWeight:600, color:"#64748b", marginBottom:4 }}>IBAN</label>
          ...
          <div>
            <label style={{ display:"block", fontSize:11, fontWeight:600, color:"#64748b", marginBottom:4 }}>Account Holder Name</label>
      ...
      {/* Account info */}
      <div style={{ background:"#0d0d0d", borderRadius:14, padding:mob?16:22, border:"1px solid rgba(255,255,255,0.06)", marginBottom:16, boxSizing:"border-box" }}>
        <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 12px", color:"#ffffff" }}>Account Details</h3>
        <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"1fr 1fr 1fr", gap:mob?10:16 }}>
          <div style={{ padding:14, background:"rgba(255,255,255,0.03)", borderRadius:10, textAlign:"center" }}>
            <div style={{ fontSize:10, fontWeight:600, color:"#64748b", marginBottom:4 }}>MEMBER SINCE</div>
            <div style={{ fontSize:13, fontWeight:600, color:"#ffffff" }}>{u.joined || "—"}</div>
          </div>
          <div style={{ padding:14, background:"rgba(255,255,255,0.03)", borderRadius:10, textAlign:"center" }}>
            <div style={{ fontSize:10, fontWeight:600, color:"#64748b", marginBottom:4 }}>REFERRED BY</div>
            <div style={{ fontSize:13, fontWeight:600, color:"#ffffff" }}>{u.referredBy || "Direct"}</div>
          </div>
          <div style={{ padding:14, background:"rgba(255,255,255,0.03)", borderRadius:10, textAlign:"center" }}>
            <div style={{ fontSize:10, fontWeight:600, color:"#64748b", marginBottom:4 }}>SUBSCRIPTION</div>
      ...
      {/* Change Password */}
      <div style={{ background:"#0d0d0d", borderRadius:14, padding:mob?16:22, border:"1px solid rgba(255,255,255,0.06)", marginBottom:16, boxSizing:"border-box" }}>
        <h3 style={{ fontSize:14, fontWeight:700, margin:"0 0 6px", color:"#ffffff" }}>Change Password</h3>
        <p style={{ fontSize:12, color:"#64748b", marginBottom:16 }}>Set a new password for your account. This device stays signed in; other sessions will need to log in again.</p>
        ...
        <div style={{ display:"grid", gridTemplateColumns:mob?"1fr":"1fr 1fr 1fr", gap:mob?12:16 }}>
          <div>
            <label style={{ display:"block", fontSize:11, fontWeight:600, color:"#64748b", marginBottom:4 }}>Current Password</label>
            <PasswordInput value={currentPw} onChange={function(e){setCurrentPw(e.target.value);setPwMsg(null)}} placeholder="Current password" />
          </div>
          <div>
            <label style={{ display:"block", fontSize:11, fontWeight:600, color:"#64748b", marginBottom:4 }}>New Password</label>
            <PasswordInput value={newPw} onChange={function(e){setNewPw(e.target.value);setPwMsg(null)}} placeholder="Min. 8 characters" />
          </div>
          <div>
            <label style={{ display:"block", fontSize:11, fontWeight:600, color:"#64748b", marginBottom:4 }}>Confirm New Password</label>
      ...
      {/* Danger Zone */}
      <div style={{ background:"#0d0d0d", borderRadius:14, padding:mob?16:22, border:"1px solid rgba(248,113,113,0.2)", boxSizing:"border-box" }}>
        ...
        <div style={{ display:"flex", flexDirection:mob?"column":"row", gap:12 }}>
          <button onClick={function(){setShowCancel(true)}} style={{ padding:"10px 20px", borderRadius:8, border:"1px solid rgba(248,113,113,0.2)", background:"#0d0d0d", fontSize:12, fontWeight:600, color:"#f87171", cursor:"pointer" }}>{cancelled ? "Subscription Cancelled" : "Cancel Subscription"}</button>
        </div>
      </div>
    </div>
  );
